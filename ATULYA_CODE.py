...
import imutils
import math
aruco1=(cv2.imread("Ha_ID3.jpg"))
aruco2=(cv2.imread("HaHa_ID4.jpg"))
aruco3= (cv2.imread("LMAO_ID1.jpg"))
aruco4 = (cv2.imread("XD_ID2.jpg "))
shapes = (cv2.imread("CVtask.jpg"))
...
for cnt in cont:
    ...
    if len(approx) == 4:
        if float(w)/h >= 0.95 and float(w)/h <= 1.05:
            cv2.drawContours(img2, [approx], 0, (0, 0, 0), -1)
            ...
            dy = int(dy)
            blank = np.zeros(np.shape(img2[y:y+h , x:x+w]) , np.uint8)

            aru = cv2.resize(aruco_Coords(aruco4),(dx,dy))
            ...
            cv2.waitKey(0)
            img2[y:y+h,x:x+w] = img2[y:y+h,x:x+w] + blank
